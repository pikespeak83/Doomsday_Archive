// Vendored from reactbits.dev (ASCIIText, MIT), itself ported from
// https://codepen.io/JuanFuentes/pen/eYEeoyE
// Grid-down adjustments: Google Fonts import removed (uses the bundled
// Share Tech Mono), gradient parametrized, defaults tuned DCI green.
...
class AsciiFilter {
  ...
  hue() {
    ...
    this.domElement.style.filter = `hue-rotate(${(this.deg * 0.2).toFixed(1)}deg)`;
  }
...
class CanvasTxt {
  constructor(txt, { fontSize = 200, fontFamily = FONT_FAMILY, color = "#d9ffc4" } = {}) {
    this.canvas = document.createElement("canvas");
    this.context = this.canvas.getContext("2d");
    this.txt = txt;
...
class CanvAscii {
  ...
  async init() {
    try {
      await document.fonts.load(`600 200px "Share Tech Mono"`);
    } catch {
      // fall back silently
    }
...
export default function ASCIIText({
  ...
  planeBaseHeight = 8,
  enableWaves = true,
  gradient = "radial-gradient(circle, #b6ff6a 0%, #7dff3f 50%, #3f8a1f 100%)"
}) {
